...
* Creating features
  ...
  components/:
  #+BEGIN_EXAMPLE
  drush @proj_dev help features-components
  drush @proj_dev features-components '%:%' > features-components.txt
  #+END_EXAMPLE
  ...
  the layout and look of a freshly installed Myproject site):
  #+BEGIN_EXAMPLE
  variable:theme_myproject_settings
  ...
  command like this:
  #+BEGIN_EXAMPLE
  drush @proj_dev features-export \
  --destination=profiles/myproject/modules/features \
  proj_layout $(cat layout_components)
  #+END_EXAMPLE
  It creates a feature on the directory:
  ~/var/www/proj_dev/profiles/myproject/modules/features/proj_layout/~

  Other features that I have created are: /proj_disqus/,
  /proj_invite/, /proj_sharethis/,
  /proj_captcha/, /proj_drupalchat/,
  /proj_janrain/, /proj_simplenews/,
  /proj_mass_contact/, /proj_googleanalytics/, etc.  In
  ...
  ### after being created (for example proj_layout).

  drush="drush --yes @proj_dev"
  destination="profiles/myproject/modules/features"

  function create_feature
  {
      components=$1
      feature_name="proj_$(basename $components)"
...
* Customizing features manually
  ...
  feature I added the file ~proj_layout.install~, which uses
  *hook_enable()* to make further customizations after the feature is
  ...
  function _proj_layout_block_settings() {
    ...
    db_query("UPDATE {block} SET title='Navigation'
	      WHERE theme='myproject' AND delta='navig-menu'");

    // remove the title of the block user-menu
    db_query("UPDATE {block} SET title='<none>'
	      WHERE theme='myproject' AND delta='user-menu'");
...
* Making features configurable
  ...
  installed when the (myproject) profile is installed, and some
  ...
  administrator/maintainer of the (Myproject) application an easy
  ...
  For example, for the feature /proj_googleanalytics/, I have
  added this line on ~proj_googleanalytics.module~:
  #+BEGIN_EXAMPLE
  include_once 'proj_googleanalytics.admin.inc';
  #+END_EXAMPLE
  I have also created the file ~proj_googleanalytics.admin.inc~
   ...
   */
  function proj_googleanalytics_menu() {
    $items = array();
    $items['admin/config/proj/googleanalytics'] = array(
      'title'            => 'GoogleAnalytics',
      'description'      => 'Private GoogleAnalytics settings.',
      'page callback'    => 'drupal_get_form',
      'page arguments'   => array('proj_googleanalytics_config'),
   ...
   */
  function proj_googleanalytics_config() {
    $form = array();
  ...
  section of Myproject configurations. This admin/config page
...
* Saving and restoring private variables
  ...
  works like this:
  #+BEGIN_EXAMPLE
  features/save-private-vars.sh @proj_dev
  drush @proj_dev php-script restore-private-vars.php
  #+END_EXAMPLE
