...
### to run it only on a test installation of Myproject.
...
if [ "$stdout" != '' ]
then
    php scripts/run-tests.sh --url "$URL" --verbose --color myproject
else
    php scripts/run-tests.sh --url "$URL" --verbose myproject > $output_file
    echo
